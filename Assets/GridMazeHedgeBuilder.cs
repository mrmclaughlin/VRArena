using System.Collections;
using System.Collections.Generic;
using UnityEngine;

// *** Journey phases: Inbound = heading to center, Outbound = heading back to entrance ***
public enum MazeJourneyPhase { Inbound, Outbound }

[ExecuteAlways]
public class GridMazeHedgeBuilder : MonoBehaviour
{
    [Header("References")]
    public Transform gymCenter;
    public Transform worldRoot;
    public GameObject hedgePrefab;

    [Header("Prefab orientation")]
    [Tooltip("Enable if hedge LENGTH runs along local X. Disable if along local Z.")]
    public bool hedgeLengthAlongLocalX = true;

    [Header("Geometry (meters)")]
    public float cellSize = 1.5f;
    public float footprintWidth = 24f;
    public float footprintLength = 15f;

    [Header("Maze shape")]
    public float outerMargin = 0.3f;

    [Range(0f, 0.5f)]
    public float braidFactor = 0.0f;

    [Header("Endpoints")]
    public bool randomizeEntranceExit = false;

    [Tooltip("Entrance is SOUTH edge x cell.")]
    public int entranceX = 0;

    [Tooltip("Used only when useCenterGoal is false.")]
    public int exitX = 0;

    [Header("Final Segment: Center Clearing")]
    [Tooltip("If true, the inbound final goal is the grid center instead of the NORTH edge.")]
    public bool useCenterGoal = true;

    [Tooltip("Radius in cells around the center goal to clear of interior walls.")]
    [Min(0)]
    public int clearingCellRadius = 1;

    [Header("Segment chaining")]
    [Tooltip("Rebuild count for EACH leg (inbound and outbound use the same value).")]
    [Min(0)]
    public int rebuildsBeforeFinalEnd = 5;

    [Header("Debug: Segment Trigger Marker")]
    public bool showSegmentTriggerMarker = true;
    public float triggerMarkerHeight = 0.6f;
    public float triggerMarkerBaseRadius = 0.25f;
    private GameObject triggerMarkerObj;

    [Tooltip("Place the rebuild trigger this many CELLS before the goal.")]
    [Min(1)]
    public int triggerCellsBeforeGoal = 3;

    [Tooltip("Try up to this many maze generations to match direction continuity.")]
    [Min(1)]
    public int maxDirectionMatchAttempts = 40;

    [Tooltip("Regenerate until the next segment's first step matches the previous direction.")]
    public bool enforceDirectionContinuity = true;

    [Header("Blocking outer openings")]
    public bool blockOuterOpeningsUntilFinal = true;
    public bool openEntranceOnFirstBuild = true;

    [Header("Rebuild timing")]
    public float rebuildCooldownSeconds = 1.0f;
    public float rebuildDelaySeconds = 0.05f;

    [Header("Rebuild Trigger (HMD-based)")]
    public Vector3 segmentTriggerSize = new Vector3(1.2f, 2.2f, 1.2f);

    [Tooltip("Main Camera / HMD. If null, will try Camera.main at runtime.")]
    public Transform hmd;

    [Header("Solution Path Debug (Scene Gizmos)")]
    public bool drawSolutionPath = true;
    public bool drawCellCenters = false;

    [Header("Solution Path Visual (In-Game)")]
    public GameObject solutionBallPrefab;
    public float solutionBallHeight = 0.25f;

    [Min(1)]
    public int solutionBallEveryNthCell = 2;
    public bool alwaysPlaceBallAtStartAndEnd = true;

    [Header("Solution Path Sound Triggers")]
    public bool enableSoundTriggersOnSolutionBalls = true;

    [Min(1)]
    public int soundTriggerEveryNthBall = 3;

    public List<AudioClip> pathSoundClips = new List<AudioClip>();
    public bool loopSoundClipList = false;

    [Range(0f, 1f)]
    public float soundVolume = 1f;

    public float soundTriggerRadius = 0.6f;
    public bool soundTriggerOneShot = true;
    public float soundTriggerCooldownSeconds = 1.5f;

    [Header("Old marker option (candles / primitive spheres)")]
    public bool spawnSolutionMarkers = false;
    public float markerHeight = 0.1f;
    public GameObject oldMarker;

    [Header("Old Marker: Bidirectional Swap")]
    [Tooltip("Inbound pass: lit candle swaps TO this prefab. Outbound pass: this prefab swaps BACK to the candle.")]
    public GameObject returnMarkerPrefab;

    [Header("Old marker extinguish (HMD-based)")]
    public float oldMarkerExtinguishRadius = 0.8f;
    public float oldMarkerExtinguishCooldown = 0.25f;
    public bool oldMarkerExtinguishOnce = true;

    [Header("Build Options")]
    public bool clearBeforeBuild = true;
    public bool buildOnStart = false;

    // ------------------------------------------------------------------ //
    //  Private state
    // ------------------------------------------------------------------ //

    // Walls bitmask: 1=N, 2=E, 4=S, 8=W
    private int[,] walls;
    private bool[,] visited;

    private int cellsX, cellsY;
    private Vector3 origin;
    private List<Vector3> solutionWorldPoints = new List<Vector3>();
    private List<Vector2Int> solutionPathCells = new List<Vector2Int>();

    private Transform solutionBallRoot;
    private GameObject segmentTriggerObj;

    private int endpointA_SouthX;
    private int endpointB_NorthX;

    private int rebuildsDone = 0;
    private float nextAllowedRebuildTime = -999f;

    private bool useForcedStartCell = false;
    private Vector2Int forcedStartCell;

    private bool useDesiredFirstStep = false;
    private Vector2Int desiredFirstStepDir;

    // *** Journey phase ***
    private MazeJourneyPhase journeyPhase = MazeJourneyPhase.Inbound;

    // Sound bag
    [SerializeField] private bool reshuffleEachLoop = true;
    private List<AudioClip> _soundBag = new List<AudioClip>();
    private int _soundBagIndex = 0;
    private int _soundBagSignature = 0;

    private Transform runtimeRoot;

    // ------------------------------------------------------------------ //
    //  Unity lifecycle
    // ------------------------------------------------------------------ //

    void Start()
    {
        if (!Application.isPlaying) return;
        if (buildOnStart)
        {
            InitializeEndpointsIfNeeded();
            Build();
        }
    }

    void InitializeEndpointsIfNeeded()
    {
        endpointA_SouthX = entranceX;
        endpointB_NorthX = exitX;
    }

    // ------------------------------------------------------------------ //
    //  Runtime root
    // ------------------------------------------------------------------ //

    private Transform GetOrCreateRuntimeRoot()
    {
        if (runtimeRoot != null) return runtimeRoot;
        var t = worldRoot.Find("__Runtime");
        if (t != null) runtimeRoot = t;
        else
        {
            runtimeRoot = new GameObject("__Runtime").transform;
            runtimeRoot.SetParent(worldRoot, true);
        }
        return runtimeRoot;
    }

    // ------------------------------------------------------------------ //
    //  Clear
    // ------------------------------------------------------------------ //

    [ContextMenu("Clear WorldRoot")]
    public void ClearWorldRoot()
    {
        if (worldRoot == null) return;
        for (int i = worldRoot.childCount - 1; i >= 0; i--)
        {
            Transform child = worldRoot.GetChild(i);
            if (child.name == "__Runtime") continue;

            // During the outbound RETRACE (rebuildsDone == 0) preserve markers so
            // the player can walk back past the existing blue cubes swapping to candles.
            // Once the first outbound rebuild fires (rebuildsDone > 0) old markers are
            // at wrong positions and must be destroyed along with everything else.
            // During outbound retrace preserve markers so the player walks back
            // past blue markers (which turn white as they pass).
            if (journeyPhase == MazeJourneyPhase.Outbound && rebuildsDone == 0)
            {
                if (child.GetComponent<OldMarkerExtinguish>() != null) continue;
            }

#if UNITY_EDITOR
            if (!Application.isPlaying) DestroyImmediate(child.gameObject);
            else Destroy(child.gameObject);
#else
            Destroy(child.gameObject);
#endif
        }
    }

    // ------------------------------------------------------------------ //
    //  Sound bag
    // ------------------------------------------------------------------ //

    private int ComputeClipSignature(List<AudioClip> clips)
    {
        unchecked
        {
            int hash = 17;
            hash = hash * 31 + (clips?.Count ?? 0);
            if (clips != null)
                for (int i = 0; i < clips.Count; i++)
                    hash = hash * 31 + (clips[i] ? clips[i].GetInstanceID() : 0);
            return hash;
        }
    }

    private void EnsureSoundBag()
    {
        if (pathSoundClips == null || pathSoundClips.Count == 0)
        {
            _soundBag.Clear(); _soundBagIndex = 0; _soundBagSignature = 0;
            return;
        }
        int sig = ComputeClipSignature(pathSoundClips);
        if (_soundBag.Count == 0 || sig != _soundBagSignature)
        {
            _soundBagSignature = sig;
            _soundBag = new List<AudioClip>(pathSoundClips);
            Shuffle(_soundBag);
            _soundBagIndex = 0;
        }
        if (_soundBagIndex >= _soundBag.Count)
        {
            _soundBagIndex = 0;
            if (reshuffleEachLoop) Shuffle(_soundBag);
        }
    }

    private AudioClip GetNextPathClip_NoRepeats()
    {
        if (!loopSoundClipList) return null;
        EnsureSoundBag();
        if (_soundBag.Count == 0) return null;
        AudioClip clip = _soundBag[_soundBagIndex];
        _soundBagIndex++;
        return clip;
    }

    private void Shuffle(List<AudioClip> list)
    {
        for (int i = list.Count - 1; i > 0; i--)
        {
            int j = Random.Range(0, i + 1);
            (list[i], list[j]) = (list[j], list[i]);
        }
    }

    // ------------------------------------------------------------------ //
    //  Build
    // ------------------------------------------------------------------ //

    [ContextMenu("Build Grid Maze (Hedges)")]
    public void Build()
    {
        if (gymCenter == null || worldRoot == null || hedgePrefab == null)
        {
            Debug.LogError("GridMazeHedgeBuilder: Assign gymCenter, worldRoot, hedgePrefab.");
            return;
        }

        // Tell all live markers which phase we are in BEFORE clearing the world.
        // On outbound rebuilds this re-arms the trigger on existing swap instances
        // without destroying them (RelightAllOldMarkers is only called on FullReset).
        NotifyMarkersOfPhase();

        if (clearBeforeBuild) ClearWorldRoot();

        if (hmd == null && Camera.main != null)
            hmd = Camera.main.transform;

        // Compute cells
        float usableW = Mathf.Max(0.1f, footprintWidth  - outerMargin * 2f);
        float usableL = Mathf.Max(0.1f, footprintLength - outerMargin * 2f);

        cellsX = Mathf.Max(2, Mathf.FloorToInt(usableW / cellSize));
        cellsY = Mathf.Max(2, Mathf.FloorToInt(usableL / cellSize));

        float mazeW = cellsX * cellSize;
        float mazeL = cellsY * cellSize;

        origin = gymCenter.position + new Vector3(-mazeW * 0.5f, 0f, -mazeL * 0.5f);

        // Randomize entrance/exit once per full loop (only on very first inbound build)
        if (randomizeEntranceExit && journeyPhase == MazeJourneyPhase.Inbound
            && rebuildsDone == 0 && !useForcedStartCell)
        {
            endpointA_SouthX = Random.Range(0, cellsX);
            endpointB_NorthX = Random.Range(0, cellsX);
        }

        endpointA_SouthX = Mathf.Clamp(endpointA_SouthX, 0, cellsX - 1);
        endpointB_NorthX = Mathf.Clamp(endpointB_NorthX, 0, cellsX - 1);

        bool isFinalSegment = (rebuildsDone >= rebuildsBeforeFinalEnd);

        // ---------------------------------------------------------------
        // Decide start and goal based on phase
        // ---------------------------------------------------------------
        Vector2Int centerCell = new Vector2Int(cellsX / 2, cellsY / 2);
        Vector2Int southCell  = new Vector2Int(endpointA_SouthX, 0);

        Vector2Int startCell;
        if (useForcedStartCell)
            startCell = ClampCell(forcedStartCell);
        else
            startCell = (journeyPhase == MazeJourneyPhase.Inbound) ? southCell : centerCell;

        Vector2Int goalCell;
        if (isFinalSegment)
        {
            if (journeyPhase == MazeJourneyPhase.Inbound)
            {
                // Final inbound segment: land in the center clearing
                goalCell = useCenterGoal ? centerCell : new Vector2Int(endpointB_NorthX, cellsY - 1);
            }
            else
            {
                // Final outbound segment: exit back through south entrance
                goalCell = southCell;
            }
        }
        else
        {
            goalCell = PickIntermediateGoal(startCell);
        }

        // ---------------------------------------------------------------
        // Generate maze with retries
        // ---------------------------------------------------------------
        int attempts = Mathf.Max(1, maxDirectionMatchAttempts);
        bool built = false;

        for (int a = 0; a < attempts; a++)
        {
            GeneratePerfectMaze(cellsX, cellsY);
            if (braidFactor > 0f) BraidMaze(cellsX, cellsY, braidFactor);

            // Entrance (south wall opening):
            //   Inbound  - open on very first segment only
            //   Outbound - always keep open so player can exit
            bool openEntrance;
            if (journeyPhase == MazeJourneyPhase.Inbound)
                openEntrance = !blockOuterOpeningsUntilFinal ||
                               (!useForcedStartCell && rebuildsDone == 0 && openEntranceOnFirstBuild);
            else
                openEntrance = true; // outbound always needs south open to exit

            // North exit wall:
            //   Only open on a non-center inbound final segment (legacy mode)
            bool openExit = !blockOuterOpeningsUntilFinal ||
                            (isFinalSegment && journeyPhase == MazeJourneyPhase.Inbound && !useCenterGoal);

            CarveSouthNorthOpenings(endpointA_SouthX, endpointB_NorthX, openEntrance, openExit);

            // Carve center clearing on final inbound segment
            if (isFinalSegment && journeyPhase == MazeJourneyPhase.Inbound
                && useCenterGoal && clearingCellRadius > 0)
                CarveClearing(goalCell, clearingCellRadius);

            if (!ComputeSolutionPathCells(startCell, goalCell))
                continue;

            if (enforceDirectionContinuity && useDesiredFirstStep)
                if (!DoesFirstStepMatchDesired(startCell))
                    continue;

            built = true;
            break;
        }

        if (!built)
        {
            Debug.LogWarning("Could not match direction continuity within attempt limit. Using last generation.");
            GeneratePerfectMaze(cellsX, cellsY);
            if (braidFactor > 0f) BraidMaze(cellsX, cellsY, braidFactor);

            bool openEntrance = journeyPhase == MazeJourneyPhase.Outbound ||
                                !blockOuterOpeningsUntilFinal ||
                                (!useForcedStartCell && rebuildsDone == 0 && openEntranceOnFirstBuild);

            bool openExit = !blockOuterOpeningsUntilFinal ||
                            (isFinalSegment && journeyPhase == MazeJourneyPhase.Inbound && !useCenterGoal);

            CarveSouthNorthOpenings(endpointA_SouthX, endpointB_NorthX, openEntrance, openExit);

            if (isFinalSegment && journeyPhase == MazeJourneyPhase.Inbound
                && useCenterGoal && clearingCellRadius > 0)
                CarveClearing(goalCell, clearingCellRadius);

            ComputeSolutionPathCells(startCell, goalCell);
        }

        BuildWallsAsHedges(cellsX, cellsY);
        BuildSolutionWorldPointsFromCells();
        SpawnSolutionBallsIfNeeded();

        if (spawnSolutionMarkers)
        {
            if (journeyPhase == MazeJourneyPhase.Inbound)
            {
                // Inbound: spawn fresh candles in their default lit state.
                SpawnSolutionMarkers();
            }
            else if (rebuildsDone > 0)
            {
                // Outbound rebuild: old markers are gone (destroyed by ClearWorldRoot).
                // Spawn fresh markers pre-swapped to the return prefab (blue cube)
                // so the player sees blue cubes guiding them out. Walking past each
                // one swaps it back to a candle.
                SpawnSolutionMarkersAsReturn();
            }
            // rebuildsDone == 0 outbound = retrace phase; existing markers preserved, no spawn.
        }

        PlaceSegmentTrigger(isFinalSegment);

        useDesiredFirstStep = false;

        Debug.Log($"Maze built. phase={journeyPhase} segment={rebuildsDone}/{rebuildsBeforeFinalEnd} " +
                  $"final={isFinalSegment} start={startCell} goal={goalCell} pathLen={solutionPathCells.Count}");
    }

    // ------------------------------------------------------------------ //
    //  Trigger notification
    // ------------------------------------------------------------------ //

    public void NotifyReachedSegmentTrigger()
    {
        if (!Application.isPlaying) return;

        Debug.Log($"[MazeTrigger] NotifyReachedSegmentTrigger called. time={Time.time:F2} allowedAt={nextAllowedRebuildTime:F2} phase={journeyPhase} rebuildsDone={rebuildsDone} rebuildsBeforeFinalEnd={rebuildsBeforeFinalEnd}");

        if (Time.time < nextAllowedRebuildTime)
        {
            Debug.Log($"[MazeTrigger] BLOCKED by cooldown. {nextAllowedRebuildTime - Time.time:F2}s remaining.");
            return;
        }
        nextAllowedRebuildTime = Time.time + rebuildCooldownSeconds;

        bool isFinalSegment = (rebuildsDone >= rebuildsBeforeFinalEnd);
        Debug.Log($"[MazeTrigger] isFinalSegment={isFinalSegment}");

        if (isFinalSegment)
        {
            if (journeyPhase == MazeJourneyPhase.Inbound)
            {
                // Reached center clearing — flip to outbound but DON'T rebuild yet.
                // The current maze stays intact. The player must retrace the full
                // inbound final segment back to its START cell. We place the trigger
                // there so the first outbound rebuild only fires once they have walked
                // the entire reversed path back out of the final segment.
                Debug.Log("CENTER reached — switching to Outbound. Player must retrace final segment.");
                journeyPhase        = MazeJourneyPhase.Outbound;
                rebuildsDone        = 0;
                useDesiredFirstStep = false;

                // solutionPathCells[0] is the cell where the player ENTERED this
                // final inbound segment — the far end of the reversed path.
                Vector2Int retraceEndCell = solutionPathCells.Count > 0
                    ? solutionPathCells[0]
                    : ClampCell(new Vector2Int(endpointA_SouthX, 0));

                forcedStartCell    = retraceEndCell;
                useForcedStartCell = true;

                // Notify markers to flip to outbound swap direction.
                NotifyMarkersOfPhase();

                // Place the trigger at the far end of the reversed path.
                // No rebuild happens until the player walks there.
                PlaceClearingEdgeTrigger(retraceEndCell);

                // Clear cooldown so the trigger can fire immediately when reached.
                nextAllowedRebuildTime = -999f;
            }
            else
            {
                // Reached south entrance on outbound — loop the whole experience.
                Debug.Log("SOUTH ENTRANCE reached — looping maze from scratch.");
                FullReset();
                StartCoroutine(RebuildAfterDelay());
            }
            return;
        }

        // Mid-segment rebuild (same logic for both phases)
        Vector2Int triggerCell = GetCurrentTriggerCell();
        forcedStartCell    = triggerCell;
        useForcedStartCell = true;

        if (enforceDirectionContinuity)
        {
            Vector2Int dir = GetDirectionAtTriggerCell(triggerCell);
            if (dir != Vector2Int.zero)
            {
                desiredFirstStepDir = dir;
                useDesiredFirstStep = true;
            }
        }

        rebuildsDone++;
        StartCoroutine(RebuildAfterDelay());
    }

    // ------------------------------------------------------------------ //
    //  Clearing edge helpers
    // ------------------------------------------------------------------ //

    /// <summary>
    /// Places the segment trigger at a specific cell without triggering a maze rebuild.
    /// Used when flipping to outbound: the trigger is placed at the far end of the
    /// reversed path (the inbound segment start) so the first outbound rebuild only
    /// fires once the player has fully retraced the final inbound segment.
    /// </summary>
    void PlaceClearingEdgeTrigger(Vector2Int edgeCell)
    {
        Transform rt = GetOrCreateRuntimeRoot();

        if (segmentTriggerObj == null)
        {
            segmentTriggerObj = new GameObject("MazeSegmentTrigger");
            segmentTriggerObj.transform.SetParent(rt, true);
            segmentTriggerObj.AddComponent<BoxCollider>().isTrigger = true;
            segmentTriggerObj.AddComponent<MazeEndTrigger>().builder = this;
        }

        segmentTriggerObj.GetComponent<BoxCollider>().size = segmentTriggerSize;

        // Disable before moving so stale position doesn't cause a spurious fire.
        segmentTriggerObj.SetActive(false);

        var trig          = segmentTriggerObj.GetComponent<MazeEndTrigger>();
        trig.hmd          = hmd;
        trig.cooldownSeconds = rebuildCooldownSeconds;
        trig.triggerCell  = edgeCell;

        Vector3 triggerPos = CellCenterWorld(edgeCell.x, edgeCell.y)
                           + Vector3.up * (segmentTriggerSize.y * 0.5f);
        segmentTriggerObj.transform.position = triggerPos;
        trig.ForceOutside();

        segmentTriggerObj.SetActive(true);

        // Update debug marker position too.
        if (showSegmentTriggerMarker && triggerMarkerObj != null)
        {
            float h = Mathf.Max(0.1f, triggerMarkerHeight);
            float r = Mathf.Max(0.05f, triggerMarkerBaseRadius);
            triggerMarkerObj.transform.position  = new Vector3(triggerPos.x, origin.y + h * 0.5f, triggerPos.z);
            triggerMarkerObj.transform.localScale = new Vector3(r * 2f, h * 0.5f, r * 2f);
            triggerMarkerObj.SetActive(showSegmentTriggerMarker);
        }

        Debug.Log($"Clearing-edge trigger placed at cell {edgeCell} pos={triggerPos}");
    }

    // Resets ALL state so the maze restarts fresh from the beginning
    private void FullReset()
    {
        journeyPhase        = MazeJourneyPhase.Inbound;
        rebuildsDone        = 0;
        useForcedStartCell  = false;
        useDesiredFirstStep = false;
        nextAllowedRebuildTime = -999f;

        // Re-randomize entrance if needed
        if (randomizeEntranceExit)
        {
            endpointA_SouthX = Random.Range(0, cellsX);
            endpointB_NorthX = Random.Range(0, cellsX);
        }
        else
        {
            endpointA_SouthX = entranceX;
            endpointB_NorthX = exitX;
        }

        // Reset all marker swap state so candles are lit again for the new loop
        RelightAllOldMarkers();
    }

    private IEnumerator RebuildAfterDelay()
    {
        if (rebuildDelaySeconds > 0f)
            yield return new WaitForSeconds(rebuildDelaySeconds);
        Build();
    }

    // ------------------------------------------------------------------ //
    //  Intermediate goal picking
    // ------------------------------------------------------------------ //

    Vector2Int PickIntermediateGoal(Vector2Int startCell)
    {
        Vector2Int dir = useDesiredFirstStep ? desiredFirstStepDir : Vector2Int.up;

        // Outbound: bias toward south to eventually funnel back to entrance
        if (journeyPhase == MazeJourneyPhase.Outbound && !useDesiredFirstStep)
            dir = Vector2Int.down;

        if (dir == Vector2Int.up)    return new Vector2Int(Random.Range(0, cellsX), cellsY - 1);
        if (dir == Vector2Int.down)  return new Vector2Int(Random.Range(0, cellsX), 0);
        if (dir == Vector2Int.right) return new Vector2Int(cellsX - 1, Random.Range(0, cellsY));
        /* left */                   return new Vector2Int(0,           Random.Range(0, cellsY));
    }

    // ------------------------------------------------------------------ //
    //  Maze generation
    // ------------------------------------------------------------------ //

    void GeneratePerfectMaze(int w, int h)
    {
        walls   = new int[w, h];
        visited = new bool[w, h];

        for (int y = 0; y < h; y++)
        for (int x = 0; x < w; x++)
            walls[x, y] = 1 | 2 | 4 | 8;

        var stack = new Stack<Vector2Int>();
        Vector2Int start = new Vector2Int(Random.Range(0, w), Random.Range(0, h));
        visited[start.x, start.y] = true;
        stack.Push(start);

        while (stack.Count > 0)
        {
            var cur       = stack.Peek();
            var neighbors = UnvisitedNeighbors(cur, w, h);

            if (neighbors.Count == 0) { stack.Pop(); continue; }

            var next = neighbors[Random.Range(0, neighbors.Count)];
            RemoveWallBetween(cur, next);
            visited[next.x, next.y] = true;
            stack.Push(next);
        }
    }

    List<Vector2Int> UnvisitedNeighbors(Vector2Int c, int w, int h)
    {
        var list = new List<Vector2Int>(4);
        TryAdd(c.x, c.y + 1); TryAdd(c.x + 1, c.y);
        TryAdd(c.x, c.y - 1); TryAdd(c.x - 1, c.y);
        return list;

        void TryAdd(int x, int y)
        {
            if (x < 0 || y < 0 || x >= w || y >= h) return;
            if (!visited[x, y]) list.Add(new Vector2Int(x, y));
        }
    }

    void RemoveWallBetween(Vector2Int a, Vector2Int b)
    {
        int dx = b.x - a.x, dy = b.y - a.y;
        if      (dx ==  1) { walls[a.x, a.y] &= ~2; walls[b.x, b.y] &= ~8; }
        else if (dx == -1) { walls[a.x, a.y] &= ~8; walls[b.x, b.y] &= ~2; }
        else if (dy ==  1) { walls[a.x, a.y] &= ~1; walls[b.x, b.y] &= ~4; }
        else if (dy == -1) { walls[a.x, a.y] &= ~4; walls[b.x, b.y] &= ~1; }
    }

    void CarveSouthNorthOpenings(int entranceXCell, int exitXCell, bool openEntrance, bool openExit)
    {
        for (int x = 0; x < cellsX; x++)
        {
            walls[x, 0]          |= 4;
            walls[x, cellsY - 1] |= 1;
        }
        for (int y = 0; y < cellsY; y++)
        {
            walls[0, y]          |= 8;
            walls[cellsX - 1, y] |= 2;
        }

        entranceXCell = Mathf.Clamp(entranceXCell, 0, cellsX - 1);
        exitXCell     = Mathf.Clamp(exitXCell,     0, cellsX - 1);

        if (openEntrance) walls[entranceXCell, 0]          &= ~4;
        if (openExit)     walls[exitXCell,     cellsY - 1] &= ~1;
    }

    void CarveClearing(Vector2Int centerCell, int cellRadius)
    {
        for (int dy = -cellRadius; dy <= cellRadius; dy++)
        for (int dx = -cellRadius; dx <= cellRadius; dx++)
        {
            int cx = centerCell.x + dx;
            int cy = centerCell.y + dy;
            if (cx < 0 || cy < 0 || cx >= cellsX || cy >= cellsY) continue;

            if (cy + 1 < cellsY) { walls[cx, cy] &= ~1; walls[cx, cy + 1] &= ~4; }
            if (cx + 1 < cellsX) { walls[cx, cy] &= ~2; walls[cx + 1, cy] &= ~8; }
            if (cy - 1 >= 0)     { walls[cx, cy] &= ~4; walls[cx, cy - 1] &= ~1; }
            if (cx - 1 >= 0)     { walls[cx, cy] &= ~8; walls[cx - 1, cy] &= ~2; }
        }
    }

    void BraidMaze(int w, int h, float factor)
    {
        for (int y = 0; y < h; y++)
        for (int x = 0; x < w; x++)
        {
            int openings = 0;
            int m = walls[x, y];
            if ((m & 1) == 0) openings++;
            if ((m & 2) == 0) openings++;
            if ((m & 4) == 0) openings++;
            if ((m & 8) == 0) openings++;

            if (openings == 1 && Random.value < factor)
            {
                var candidates = new List<Vector2Int>();
                if (y + 1 < h && (m & 1) != 0) candidates.Add(new Vector2Int(x, y + 1));
                if (x + 1 < w && (m & 2) != 0) candidates.Add(new Vector2Int(x + 1, y));
                if (y - 1 >= 0 && (m & 4) != 0) candidates.Add(new Vector2Int(x, y - 1));
                if (x - 1 >= 0 && (m & 8) != 0) candidates.Add(new Vector2Int(x - 1, y));
                if (candidates.Count > 0)
                    RemoveWallBetween(new Vector2Int(x, y), candidates[Random.Range(0, candidates.Count)]);
            }
        }
    }

    // ------------------------------------------------------------------ //
    //  Wall geometry
    // ------------------------------------------------------------------ //

    void BuildWallsAsHedges(int w, int h)
    {
        for (int y = 0; y < h; y++)
        for (int x = 0; x < w; x++)
        {
            int m = walls[x, y];
            Vector3 cellCenter = origin + new Vector3((x + 0.5f) * cellSize, 0f, (y + 0.5f) * cellSize);
            float half = cellSize * 0.5f;

            if ((m & 1) != 0) SpawnWall(cellCenter + new Vector3(0f,   0f,  half), Quaternion.identity);
            if ((m & 2) != 0) SpawnWall(cellCenter + new Vector3(half,  0f,  0f),   Quaternion.Euler(0f, 90f, 0f));
            if (y == 0 && (m & 4) != 0) SpawnWall(cellCenter + new Vector3(0f, 0f, -half), Quaternion.identity);
            if (x == 0 && (m & 8) != 0) SpawnWall(cellCenter + new Vector3(-half, 0f, 0f), Quaternion.Euler(0f, 90f, 0f));
        }
    }

    void SpawnWall(Vector3 pos, Quaternion rot)
    {
        if (hedgeLengthAlongLocalX) rot *= Quaternion.Euler(0f, 90f, 0f);
#if UNITY_EDITOR
        GameObject h = (!Application.isPlaying)
            ? (GameObject)UnityEditor.PrefabUtility.InstantiatePrefab(hedgePrefab, worldRoot)
            : Instantiate(hedgePrefab, worldRoot);
#else
        GameObject h = Instantiate(hedgePrefab, worldRoot);
#endif
        h.transform.SetPositionAndRotation(pos, rot);
    }

    // ------------------------------------------------------------------ //
    //  Solution path BFS
    // ------------------------------------------------------------------ //

    bool ComputeSolutionPathCells(Vector2Int start, Vector2Int goal)
    {
        solutionPathCells.Clear();
        solutionWorldPoints.Clear();

        var parent = new Dictionary<Vector2Int, Vector2Int>();
        var q      = new Queue<Vector2Int>();
        var seen   = new HashSet<Vector2Int>();

        q.Enqueue(start);
        seen.Add(start);
        bool found = false;

        while (q.Count > 0)
        {
            var cur = q.Dequeue();
            if (cur == goal) { found = true; break; }
            foreach (var n in OpenNeighbors(cur))
            {
                if (seen.Contains(n)) continue;
                seen.Add(n);
                parent[n] = cur;
                q.Enqueue(n);
            }
        }

        if (!found) return false;

        var p = goal;
        solutionPathCells.Add(p);
        while (p != start) { p = parent[p]; solutionPathCells.Add(p); }
        solutionPathCells.Reverse();
        return (solutionPathCells.Count >= 2);
    }

    void BuildSolutionWorldPointsFromCells()
    {
        solutionWorldPoints.Clear();
        foreach (var c in solutionPathCells)
            solutionWorldPoints.Add(CellCenterWorld(c.x, c.y));
    }

    IEnumerable<Vector2Int> OpenNeighbors(Vector2Int c)
    {
        int x = c.x, y = c.y, m = walls[x, y];
        if ((m & 1) == 0 && y + 1 < cellsY) yield return new Vector2Int(x, y + 1);
        if ((m & 2) == 0 && x + 1 < cellsX) yield return new Vector2Int(x + 1, y);
        if ((m & 4) == 0 && y - 1 >= 0)     yield return new Vector2Int(x, y - 1);
        if ((m & 8) == 0 && x - 1 >= 0)     yield return new Vector2Int(x - 1, y);
    }

    Vector3 CellCenterWorld(int x, int y)
        => origin + new Vector3((x + 0.5f) * cellSize, 0f, (y + 0.5f) * cellSize);

    // ------------------------------------------------------------------ //
    //  Direction continuity helpers
    // ------------------------------------------------------------------ //

    bool DoesFirstStepMatchDesired(Vector2Int startCell)
    {
        if (solutionPathCells.Count < 2) return false;
        if (solutionPathCells[0] != startCell) return false;
        return (solutionPathCells[1] - solutionPathCells[0]) == desiredFirstStepDir;
    }

    Vector2Int GetDirectionAtTriggerCell(Vector2Int triggerCell)
    {
        for (int i = 0; i < solutionPathCells.Count - 1; i++)
            if (solutionPathCells[i] == triggerCell)
                return solutionPathCells[i + 1] - solutionPathCells[i];
        return Vector2Int.zero;
    }

    Vector2Int GetCurrentTriggerCell()
    {
        if (segmentTriggerObj == null) return forcedStartCell;
        var t = segmentTriggerObj.GetComponent<MazeEndTrigger>();
        return (t != null) ? t.triggerCell : forcedStartCell;
    }

    Vector2Int ClampCell(Vector2Int c)
        => new Vector2Int(Mathf.Clamp(c.x, 0, cellsX - 1), Mathf.Clamp(c.y, 0, cellsY - 1));

    // ------------------------------------------------------------------ //
    //  Segment trigger placement
    // ------------------------------------------------------------------ //

    void PlaceSegmentTrigger(bool isFinalSegment)
    {
        if (solutionPathCells == null || solutionPathCells.Count < 2) return;

        int idx = isFinalSegment
            ? solutionPathCells.Count - 1
            : Mathf.Clamp(solutionPathCells.Count - 1 - triggerCellsBeforeGoal, 1, solutionPathCells.Count - 2);

        Vector2Int triggerCell = solutionPathCells[idx];
        Vector3 triggerPos = CellCenterWorld(triggerCell.x, triggerCell.y)
                           + Vector3.up * (segmentTriggerSize.y * 0.5f);

        Transform rt = GetOrCreateRuntimeRoot();

        if (segmentTriggerObj == null)
        {
            segmentTriggerObj = new GameObject("MazeSegmentTrigger");
            segmentTriggerObj.transform.SetParent(rt, true);
            segmentTriggerObj.AddComponent<BoxCollider>().isTrigger = true;
            segmentTriggerObj.AddComponent<MazeEndTrigger>().builder = this;
        }

        segmentTriggerObj.GetComponent<BoxCollider>().size = segmentTriggerSize;

        // Disable before moving so the trigger's Update() doesn't fire on the
        // stale position while we reposition it.
        segmentTriggerObj.SetActive(false);

        var trig          = segmentTriggerObj.GetComponent<MazeEndTrigger>();
        trig.hmd          = hmd;
        trig.cooldownSeconds = rebuildCooldownSeconds;
        trig.triggerCell  = triggerCell;

        // Move first, THEN reset state — wasInside is now evaluated against
        // the new position, not the old one.
        segmentTriggerObj.transform.position = triggerPos;
        trig.ForceOutside();

        segmentTriggerObj.SetActive(true);

        if (showSegmentTriggerMarker)
        {
            if (triggerMarkerObj == null)
            {
                triggerMarkerObj = GameObject.CreatePrimitive(PrimitiveType.Cylinder);
                triggerMarkerObj.name = "SegmentTriggerMarker_CONE";
                triggerMarkerObj.transform.SetParent(rt, true);
                var c = triggerMarkerObj.GetComponent<Collider>();
                if (c) Destroy(c);
            }
            float h = Mathf.Max(0.1f, triggerMarkerHeight);
            float r = Mathf.Max(0.05f, triggerMarkerBaseRadius);
            triggerMarkerObj.transform.position  = new Vector3(triggerPos.x, origin.y + h * 0.5f, triggerPos.z);
            triggerMarkerObj.transform.localScale = new Vector3(r * 2f, h * 0.5f, r * 2f);
            triggerMarkerObj.SetActive(true);
        }
        else if (triggerMarkerObj != null)
        {
            triggerMarkerObj.SetActive(false);
        }

        Debug.Log($"SegmentTrigger placed at cell {triggerCell} (idx {idx}/{solutionPathCells.Count - 1}) pos={triggerPos}");
    }

    // ------------------------------------------------------------------ //
    //  Solution balls
    // ------------------------------------------------------------------ //

    void SpawnSolutionBallsIfNeeded()
    {
        if (solutionBallPrefab == null) return;
        if (solutionWorldPoints == null || solutionWorldPoints.Count == 0) return;

        solutionBallRoot = new GameObject("SolutionBalls").transform;
        solutionBallRoot.SetParent(worldRoot, true);

        if (hmd == null && Camera.main != null) hmd = Camera.main.transform;

        int n = Mathf.Max(1, solutionBallEveryNthCell);
        int placedBallCount = 0;

        for (int i = 0; i < solutionWorldPoints.Count; i++)
        {
            bool place = (i % n == 0) ||
                         (alwaysPlaceBallAtStartAndEnd && (i == 0 || i == solutionWorldPoints.Count - 1));
            if (!place) continue;

            Vector3 pos = solutionWorldPoints[i] + Vector3.up * solutionBallHeight;
#if UNITY_EDITOR
            GameObject orb = (!Application.isPlaying)
                ? (GameObject)UnityEditor.PrefabUtility.InstantiatePrefab(solutionBallPrefab, solutionBallRoot)
                : Instantiate(solutionBallPrefab, solutionBallRoot);
#else
            GameObject orb = Instantiate(solutionBallPrefab, solutionBallRoot);
#endif
            orb.transform.position = pos;
            placedBallCount++;

            if (enableSoundTriggersOnSolutionBalls && soundTriggerEveryNthBall > 0
                && (placedBallCount % soundTriggerEveryNthBall == 0)
                && pathSoundClips != null && pathSoundClips.Count > 0)
            {
                AudioClip chosen = GetNextPathClip_NoRepeats();
                if (chosen != null) AddSoundTriggerToOrb(orb, chosen);
            }
        }
    }

    void AddSoundTriggerToOrb(GameObject orb, AudioClip clip)
    {
        GameObject zoneObj = new GameObject("OrbSoundZone");
        zoneObj.transform.SetParent(orb.transform, false);
        zoneObj.transform.localPosition = Vector3.zero;

        var sc = zoneObj.AddComponent<SphereCollider>();
        sc.isTrigger = true;
        sc.radius    = soundTriggerRadius;

        var a = zoneObj.AddComponent<AudioSource>();
        a.playOnAwake  = false;
        a.spatialBlend = 1f;
        a.clip         = clip;
        a.volume       = soundVolume;
        a.rolloffMode  = AudioRolloffMode.Logarithmic;
        a.minDistance  = 0.25f;
        a.maxDistance  = 12f;

        if (hmd == null && Camera.main != null) hmd = Camera.main.transform;

        zoneObj.AddComponent<HmdSoundZone>()
               .Init(hmd, clip, soundVolume, soundTriggerOneShot, soundTriggerCooldownSeconds);
    }

    // ------------------------------------------------------------------ //
    //  Old markers (candles)
    // ------------------------------------------------------------------ //

    // Tell every live marker which phase we're in so they swap in the right direction
    private void NotifyMarkersOfPhase()
    {
        if (worldRoot == null) return;
        var markers = worldRoot.GetComponentsInChildren<OldMarkerExtinguish>(true);
        foreach (var m in markers)
            m.SetPhase(journeyPhase);
    }

    private void RelightAllOldMarkers()
    {
        if (worldRoot == null) return;
        var markers = worldRoot.GetComponentsInChildren<OldMarkerExtinguish>(true);
        foreach (var m in markers) m.Relight();
    }

    void SpawnSolutionMarkers()
    {
        if (hmd == null && Camera.main != null) hmd = Camera.main.transform;

        for (int i = 0; i < solutionWorldPoints.Count; i++)
        {
            GameObject s = Instantiate(oldMarker);
            s.name = $"SolutionMarker_{i}";
            s.transform.SetParent(worldRoot, true);
            s.transform.position   = solutionWorldPoints[i] + Vector3.up * markerHeight;
            s.transform.localScale = Vector3.one * 0.15f;

            var ext = s.GetComponent<OldMarkerExtinguish>();
            if (ext == null)
            {
                Debug.LogWarning("OldMarker prefab missing OldMarkerExtinguish script.");
            }
            else
            {
                ext.hmd              = hmd;
                ext.extinguishRadius = oldMarkerExtinguishRadius;
                ext.cooldownSeconds  = oldMarkerExtinguishCooldown;
                ext.extinguishOnce   = oldMarkerExtinguishOnce;
                ext.SetSwapPrefab(returnMarkerPrefab);
                ext.SetPhase(journeyPhase);
                ext.ForceInboundColour(); // always start white regardless of prefab state
            }
        }
    }

    /// <summary>
    /// Spawns markers for outbound rebuilt segments.
    /// Markers start pre-swapped (showing the return prefab / blue cube).
    /// Walking past each one swaps it back to the original candle.
    /// </summary>
    void SpawnSolutionMarkersAsReturn()
    {
        // Spawn the same marker prefab as inbound but starting in blue (outbound) colour.
        // Walking past each one turns it white, matching the retrace feedback.
        if (oldMarker == null) return;
        if (hmd == null && Camera.main != null) hmd = Camera.main.transform;

        for (int i = 0; i < solutionWorldPoints.Count; i++)
        {
            GameObject s = Instantiate(oldMarker);
            s.name = $"SolutionMarker_{i}";
            s.transform.SetParent(worldRoot, true);
            s.transform.position   = solutionWorldPoints[i] + Vector3.up * markerHeight;
            s.transform.localScale = Vector3.one * 0.15f;

            var ext = s.GetComponent<OldMarkerExtinguish>();
            if (ext == null)
            {
                Debug.LogWarning("OldMarker prefab missing OldMarkerExtinguish script.");
                continue;
            }

            ext.hmd              = hmd;
            ext.extinguishRadius = oldMarkerExtinguishRadius;
            ext.cooldownSeconds  = oldMarkerExtinguishCooldown;
            ext.extinguishOnce   = oldMarkerExtinguishOnce;

            // Start blue (already-walked colour) and arm for outbound swap back to white.
            ext.InitAsAlreadySwapped(null, journeyPhase);
        }
    }

    // ------------------------------------------------------------------ //
    //  Gizmos
    // ------------------------------------------------------------------ //

    void OnDrawGizmosSelected()
    {
        if (!drawSolutionPath) return;
        if (solutionWorldPoints == null || solutionWorldPoints.Count < 2) return;

        Gizmos.color = (journeyPhase == MazeJourneyPhase.Inbound) ? Color.cyan : Color.green;
        for (int i = 0; i < solutionWorldPoints.Count - 1; i++)
        {
            Vector3 a = solutionWorldPoints[i]     + Vector3.up * 0.05f;
            Vector3 b = solutionWorldPoints[i + 1] + Vector3.up * 0.05f;
            Gizmos.DrawLine(a, b);
        }

        if (drawCellCenters)
        {
            Gizmos.color = Color.yellow;
            foreach (var p in solutionWorldPoints)
                Gizmos.DrawSphere(p + Vector3.up * 0.05f, 0.08f);
        }
    }
}